function CommonHarWavelets = Hcp_Identify_glob_com_har_wavelets(Graph,CommonNetwork)
...
for N_i=1:NodeNum
    ...
    Phi_k=Graph(20).LocalizedRegion(N_i).Harmonics;
    err=1;       
    iter2=1;
    while err>0.0000001&&iter2<100
        Phi_increment=zeros(NodeNum,k);
        for S_j=1:SubjectNum
            Phi_increment=Phi_increment+Phi_k*Graph(S_j).LocalizedRegion(N_i).Harmonics'*Phi_k-Graph(S_j).LocalizedRegion(N_i).Harmonics;
        end
